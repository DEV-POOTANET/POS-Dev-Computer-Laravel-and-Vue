...
export default {
  ...
  methods: {
    ...
    async addProduct() {
      ...
      const token = authStore.token;
      try {
        const response = await axios.post('http://localhost:8000/api/addProduct', this.product,{
          headers: {
            Authorization: `Bearer ${token}`,
            },
        });
        ...
        this.errorMessage = error.response.data.message || 'เกิดข้อผิดพลาดในการเพิ่มสินค้า';
      }
    },
    clearForm() {
      this.product = {
